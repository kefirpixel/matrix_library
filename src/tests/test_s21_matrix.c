  ...
  if (matrix.matrix) {
    for (int i = 0; i < rows; ++i) {
      for (int j = 0; j < columns; ++j) {
        ck_assert(fabs(matrix.matrix[i][j]) < EPSILON);
      }
    }
...
START_TEST(sum_matrix_1) {
  ...
  ck_assert(fabs(R.matrix[0][0] - R2.matrix[0][0]) < EPSILON);
...
START_TEST(sum_test_INVALID_MATRIX_matrix) {
  matrix_t A, B, R;
  s21_create_matrix(1, 1, &A);
...
START_TEST(sub_test_INVALID_MATRIX_matrix) {
  matrix_t A, B, R;
  s21_create_matrix(1, 1, &A);
  s21_create_matrix(2, 1, &B);
...
START_TEST(mult_INVALID_MATRIX_matrix_1) {
  matrix_t m1 = {0};
  matrix_t m2 = {0};
...
START_TEST(mult_INVALID_MATRIX_matrix_2) {
  matrix_t m1 = {0};
  matrix_t m2 = {0};
  s21_create_matrix(5, 5, &m1);
  s21_create_matrix(6, 6, &m2);
...
START_TEST(mult_matrix_test_null) {
  ...
  ck_assert_int_eq(s21_mult_matrix(A, B, R), INVALID_MATRIX);
}
END_TEST
...
START_TEST(transpose_INVALID_MATRIX) {
  matrix_t A, B;
  s21_create_matrix(1, 1, &A);
  s21_create_matrix(1, 1, &B);
  A.columns = -1;
...
START_TEST(empty) {
  matrix_t a = {0};
  matrix_t b = {0};
  ck_assert_int_eq(s21_calc_complements(&a, &b), INVALID_MATRIX);
}
END_TEST
...
START_TEST(determinant_test_2) {
  ...
  ck_assert((fabs(B) - fabs(R)) < EPSILON);
...
START_TEST(inverse_one_by_one) {
  ...
  ck_assert_int_eq(result.matrix[0][0], 1.0 / m.matrix[0][0]);
  ck_assert_int_eq(code, OK);
  s21_remove_matrix(&m);
  s21_remove_matrix(&result);
}
...
START_TEST(inverse_INVALID_MATRIX) {
  matrix_t m = {0};
  matrix_t result = {0};
  int code = s21_inverse_matrix(&m, &result);
...
*s21_matrix_test(void) {
  ...
  tcase_add_test(tCase, create_INVALID_MATRIX);
  ...
  tcase_add_test(tCase, sum_test_INVALID_MATRIX_matrix);
  ...
  tcase_add_test(tCase, sub_test_INVALID_MATRIX_matrix);
  ...
  tcase_add_test(tCase, mult_INVALID_MATRIX_matrix_1);
  tcase_add_test(tCase, mult_INVALID_MATRIX_matrix_2);
  ...
  tcase_add_test(tCase, transpose_INVALID_MATRIX);
  ...
  tcase_add_test(tCase, inverse_INVALID_MATRIX);
...
int main() {
  ...
  int all = 0;
  int success = 0;
  Suite *list_cases[] = {s21_matrix_test(), NULL};
